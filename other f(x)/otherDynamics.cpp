...
//#include "piecewise/piecewise.cpp"
...
/// heterogeneity should be only supplied right after initialization
//function to give heterogeneity randomly
void Dynamics::apply_heterogeneity(int N1)
{
    // assigning n1 nodes a=1
...
void Dynamics::calculate_derivatives(double b, double c)
{
    for(int i=0; i<links.size(); i++)
    {
        ...
        Dx[i] = g(x[i]) + c*(nf- x[i]) + b + a[i] ;
    }
}

//function to evolve sites with unit variance Gaussian noise
